...
import { ActivityIndicator, Alert, FlatList, Linking, Modal, Platform, Pressable, StyleSheet, Text, TouchableOpacity, View } from "react-native";
...
import { trpc } from "@/lib/trpc";
 ...
 type MediaKind = "image" | "album" | "video";
 type ReplyTarget = { id: number; content: string };
 type Message = { id: number | string; conversationId: number; senderId: number; kind: "text" | "image" | "album" | "video" | "file" | "sticker"; content: string; attachmentUrl?: string | null; attachmentUrls?: string | null; attachmentName?: string | null; createdAt: string | Date; status?: "sent" | "delivered" | "read"; failed?: boolean; replyToMessageId?: number };
...
export default function ChatScreen() {
  ...
  useEffect(() => { if (!messagesQuery.data?.length) return; setOptimisticMessages((current) => current.filter((local) => !messagesQuery.data.some((server) => server.senderId === local.senderId && server.kind === local.kind && server.content === local.content && (server.attachmentUrl || null) === (local.attachmentUrl || null) && Math.abs(new Date(server.createdAt).getTime() - new Date(local.createdAt).getTime()) < 15000))); }, [messagesQuery.data]);
  const sendOptimistically = (payload: { kind: Message["kind"]; content: string; attachmentName?: string; attachmentUrl?: string; attachmentUrls?: string; replyToMessageId?: number }) => {
    const localId = `local-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
    setOptimisticMessages((current) => [...current, { id: localId, conversationId, senderId: user?.id ?? 0, createdAt: new Date().toISOString(), status: "sent", ...payload }]);
    send.mutate({ conversationId, ...payload }, { onSuccess: () => { void utils.chat.messages.invalidate({ conversationId }); void utils.chat.list.invalidate(); setReplyTarget(null); }, onError: () => setOptimisticMessages((current) => current.map((message) => message.id === localId ? { ...message, failed: true, status: "sent" } : message)) });
  };
  const sendText = (content: string) => sendOptimistically({ kind: "text", content, ...(replyTarget ? { replyToMessageId: replyTarget.id } : {}) });
  const sendAttachment = (attachment: { kind: "image" | "album" | "video" | "file" | "sticker"; name: string; uri?: string; attachmentUrls?: string[]; count?: number }) => sendOptimistically({ kind: attachment.kind, attachmentUrls: attachment.attachmentUrls ? JSON.stringify(attachment.attachmentUrls) : undefined, content: attachment.kind === "sticker" ? attachment.name : attachment.kind === "video" ? "Video" : attachment.kind === "album" ? `Đã gửi album ${attachment.count ?? 1} ảnh` : attachment.name, attachmentName: attachment.name, attachmentUrl: attachment.uri });
  ...
  const thread: Message[] = [...((messagesQuery.data ?? []) as Message[]), ...optimisticMessages].sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
  const saveMedia = async () => {
    ...
    try {
      ...
      const downloaded = await FileSystem.downloadAsync(media.attachmentUrl, localUri);
