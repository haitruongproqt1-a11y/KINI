...
export default function DiscoverScreen() {
  ...
  const sendFriendRequest = trpc.friends.send.useMutation();
  const openDirectConversation = trpc.chat.openDirect.useMutation();
  ...
  const save = async () => {
    ...
    setWorking(true);
    try {
      const saved = await nearbyApi.saveProfile({ gender, status, province, birthYear: parsed, bio, job });
      const coords = await getLocation();
      setFromProfile({ ...saved, lat: coords.latitude, lng: coords.longitude });
      setNotice("Đã lưu hồ sơ và cập nhật vị trí Tìm Quanh Đây.");
    }
  ...
  const find = useCallback(async (refreshLocation = false) => {
    let lat = profile?.lat; let lng = profile?.lng;
    if (refreshLocation || lat === null || lng === null || lat === undefined || lng === undefined) {
  ...
  const updateSelectedRelation = (relation: NearbyUser["relation"]) => {
    if (!selected) return;
    setSelected({ ...selected, relation });
    setPeople((current) => current.map((item) => item.userId === selected.userId ? { ...item, relation } : item));
  };
  const addNearbyFriend = async () => {
    if (!selected) return;
    try {
      const response = await sendFriendRequest.mutateAsync({ userId: selected.userId });
      const relation = response.accepted ? "friends" : selected.relation === "incoming" ? "friends" : "outgoing";
      updateSelectedRelation(relation);
      setNotice(response.accepted ? "Đã chấp nhận lời mời kết bạn." : "Đã gửi lời mời kết bạn.");
    } catch (error) { setNotice(error instanceof Error ? error.message : "Không thể gửi lời mời kết bạn."); }
  };
  const messageNearbyUser = async () => {
    if (!selected) return;
    if (selected.relation !== "friends") {
      setNotice(selected.relation === "outgoing" ? "Lời mời kết bạn đang chờ phản hồi." : "Hãy kết bạn trước khi nhắn tin trực tiếp.");
      return;
    }
    try {
      const conversationId = await openDirectConversation.mutateAsync({ friendUserId: selected.userId });
      setPicker(null);
      router.push(`/chat/${conversationId}` as never);
    } catch (error) { setNotice(error instanceof Error ? error.message : "Không thể mở cuộc trò chuyện."); }
  };
  ...
  const header = <>
    ...
    <View style={styles.actionRow}><TouchableOpacity onPress={reset}><Text style={styles.link}>Xóa bộ lọc</Text></TouchableOpacity><KiniButton label="Tìm" onPress={() => void find(true)} /></View>
  ...
  return <ScreenContainer>
    ...
    <KiniBottomSheet visible={picker === "province"} title={active === "me" ? "Chọn tỉnh thành" : "Lọc tỉnh thành"} onClose={() => setPicker(null)}><ProvinceOptions value={active === "me" ? province : filterProvince} onChoose={(value) => { active === "me" ? setProvince(value) : setFilterProvince(value); setPicker(null); }} /><KiniButton label={active === "me" ? "Bỏ chọn" : "Tất cả tỉnh thành"} onPress={() => { active === "me" ? setProvince("") : setFilterProvince(""); setPicker(null); }} variant="secondary" /></KiniBottomSheet>
    ...
    <KiniBottomSheet visible={picker === "detail"} title="Hồ sơ KINI" onClose={() => setPicker(null)}>{selected ? <><View style={styles.detail}><Avatar initials={initial(selected.name)} color={selected.avatarColor} imageUri={selected.avatar} size={66} /><View style={styles.flex}><Text style={styles.identityName}>{selected.name}</Text><Text style={styles.muted}>{selected.province ?? "KINI"} · {selected.distanceKm} km</Text></View></View><Text style={styles.detailBio}>{selected.bio || "Chưa có giới thiệu."}</Text><Text style={styles.small}>{[selected.gender && genders[selected.gender], selected.age && `${selected.age} tuổi`, selected.status && statuses[selected.status], selected.job].filter(Boolean).join(" · ")}</Text><View style={styles.detailActions}>{selected.relation === "friends" ? null : <KiniButton label={selected.relation === "incoming" ? "Chấp nhận kết bạn" : selected.relation === "outgoing" ? "Đã gửi lời mời" : "Kết bạn"} onPress={() => void addNearbyFriend()} disabled={selected.relation === "outgoing" || sendFriendRequest.isPending} variant="secondary" />}<KiniButton label="Nhắn tin" onPress={() => void messageNearbyUser()} disabled={openDirectConversation.isPending} /></View></> : null}</KiniBottomSheet>
...
function ProvinceOptions({ value, onChoose }: { value: string; onChoose: (value: string) => void }) { const [search, setSearch] = useState(""); const matches = vietnamProvinces.filter((item) => item.toLocaleLowerCase("vi-VN").includes(search.trim().toLocaleLowerCase("vi-VN"))); return <View style={styles.provincePicker}><TextInput value={search} onChangeText={setSearch} placeholder="Tìm tỉnh thành" placeholderTextColor={kiniColors.muted} style={styles.provinceSearch} /><ScrollView style={styles.provinceList} keyboardShouldPersistTaps="handled">{matches.map((item) => <TouchableOpacity key={item} onPress={() => onChoose(item)} style={styles.option}><Text style={[styles.optionText, value === item && styles.optionActive]}>{item}</Text>{value === item ? <MaterialIcons name="check" size={20} color={kiniColors.blue} /> : <MaterialIcons name="chevron-right" size={20} color={kiniColors.muted} />}</TouchableOpacity>)}</ScrollView></View>; }
