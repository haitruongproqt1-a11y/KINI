...
type Attachment = {
  id: string;
  kind: "image" | "album" | "video" | "file" | "sticker";
  name: string;
  uri?: string;
  contentType?: string;
  size?: number | null;
...
export function ChatComposer({ onSendText, onSendAttachment, pasteNonce = 0, replyingTo, onClearReply }: { onSendText: (value: string) => void; onSendAttachment: (attachment: Attachment) => void; pasteNonce?: number; replyingTo?: string | null; onClearReply?: () => void }) {
  ...
  const uploadAndSend = async (asset: ImagePicker.ImagePickerAsset, kind: "image" | "video" = "image") => {
    setIsUploading(true);
    try {
      const name = asset.fileName ?? `${kind}-${Date.now()}.${kind === "video" ? "mp4" : "jpg"}`;
      const uploaded = await uploadMedia(asset.uri, name, asset.mimeType ?? (kind === "video" ? "video/mp4" : "image/jpeg"), asset.fileSize);
      ...
      Alert.alert("Không thể gửi media", error instanceof Error ? error.message : "Vui lòng thử lại.");
    } finally {
  ...
  const selectImages = async (album: boolean) => {
    ...
    if (Platform.OS !== "web") {
      ...
      if (!permission.granted) { Alert.alert("Cần quyền truy cập", "KINI cần quyền thư viện ảnh để gửi hình ảnh trong cuộc trò chuyện."); return; }
    }
    const result = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ["images"], allowsMultipleSelection: album, selectionLimit: album ? 12 : 1, quality: 0.82 });
    if (result.canceled || !result.assets.length) return;
    if (album) {
      const first = result.assets[0];
      setIsUploading(true);
      try {
        const uploaded = await uploadMedia(first.uri, first.fileName ?? `album-${Date.now()}.jpg`, first.mimeType ?? "image/jpeg", first.fileSize);
        onSendAttachment({ id: `${Date.now()}-album`, kind: "album", name: "Album ảnh", uri: uploaded.url, contentType: uploaded.contentType, size: uploaded.size, count: result.assets.length });
      } catch (error) { Alert.alert("Không thể gửi album", error instanceof Error ? error.message : "Vui lòng thử lại."); } finally { setIsUploading(false); }
      return;
    }
  ...
  const selectVideo = async () => {
    ...
    if (Platform.OS !== "web") {
      const permission = await ImagePicker.requestMediaLibraryPermissionsAsync();
      if (!permission.granted) { Alert.alert("Cần quyền truy cập", "KINI cần quyền thư viện để chọn video."); return; }
    }
    ...
    if (!result.canceled && result.assets.length) await uploadAndSend(result.assets[0], "video");
  };
  ...
  const selectDocument = async () => {
    ...
    setIsUploading(true);
    try {
      const uploaded = await uploadMedia(file.uri, file.name, file.mimeType ?? "application/octet-stream", file.size);
    ...
    } catch (error) { Alert.alert("Không thể gửi tệp", error instanceof Error ? error.message : "Vui lòng thử lại."); } finally { setIsUploading(false); }
  };
  ...
  return (
    <View style={styles.wrapper}>
      ...
      {isUploading ? <View style={styles.uploading}><MaterialIcons name="cloud-upload" size={16} color={kiniColors.blue} /><Text style={styles.uploadingText}>Đang tải media lên…</Text></View> : null}
      <View style={styles.composer}>
        <TouchableOpacity disabled={isUploading} accessibilityRole="button" accessibilityLabel="Gửi ảnh, video, album hoặc tệp" onPress={() => setShowActions(true)} style={styles.control} activeOpacity={0.6}><MaterialIcons name="add-circle-outline" size={25} color={kiniColors.blue} /></TouchableOpacity>
        <TextInput editable={!isUploading} value={value} onChangeText={setValue} onSubmitEditing={send} placeholder="Nhắn tin" placeholderTextColor="#97A4B5" returnKeyType="send" style={styles.input} multiline />
        {value.trim() ? <TouchableOpacity disabled={isUploading} accessibilityRole="button" accessibilityLabel="Gửi tin nhắn" onPress={send} style={styles.send} activeOpacity={0.75}><MaterialIcons name="send" size={19} color={kiniColors.white} /></TouchableOpacity> : <TouchableOpacity disabled={isUploading} accessibilityRole="button" accessibilityLabel="Chọn sticker" onPress={() => setShowStickers(true)} style={styles.control} activeOpacity={0.6}><MaterialIcons name="sentiment-satisfied-alt" size={24} color={kiniColors.blue} /></TouchableOpacity>}
      </View>
